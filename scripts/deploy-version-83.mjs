...
async function deployV83() {
  console.log('=== DEPLOY APPS SCRIPT VERSION 83 PRODUCTION ===\n');

  console.log('1. Pushing local source files...');
  await runClasp(['push', '--force']);

  console.log('\n2. Creating version 83 on Google Apps Script...');
  const verRes = await runClasp(['version', 'v83: Add DS validation and Lesson 5 mapping in importnganhang']);
  console.log('Version creation output:', verRes.stdout);

  console.log('\n3. Deploying version 83 to production deployment ID...');
  const deployRes = await runClasp([
    'deploy',
    '--deploymentId', 'AKfycbyqejp4SzgwNsJb3QrTP76C5-6K2MYqv5T1CzPyi6KUOEEsC7GKQLCnR07i0DNbqKBL',
    '--versionNumber', '83',
    '--description', 'v83: Add DS validation and Lesson 5 mapping in importnganhang'
  ]);
  ...
  console.log('\n=== HOÀN TẤT TRIỂN KHAI VERSION 83 LÊN PRODUCTION ===');
}
